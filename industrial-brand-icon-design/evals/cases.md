...
**输入：** 用户要求按默认品牌规范生成一组工业功能图标，但未额外指定颜色或文件格式。
...
- 不出现橙色、灰色或其他第三种颜色；
- 默认输出为 PNG 格式；
- 默认使用透明背景，便于后续放到深色页面、A+、Banner 或功能卡中；
- 白色线条与白色实心面保持统一，不因为取消橙色而退化为纯线稿；
- 若用户当次明确要求其他颜色或背景，可作为 A级｜单次参考临时覆盖，不自动修改长期品牌默认。

**失败条件：**

- 默认输出仍出现橙色强调；
- 输出成 JPG、SVG 或其他格式而未同时提供 PNG；
- 白色图标被固定在不必要的黑色背景中，导致无法灵活复用；
- 为了纯白而丢失线面层级、负空间或可读性。
