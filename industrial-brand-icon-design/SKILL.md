...
白色单色输出仍必须保持“线面结合”，不能因为取消颜色而退化为纯线稿。
...
检查：语义、主体、身份特征、辅助元素、内部空/密、缩略可读性、是否保持纯白单色和正确 PNG 输出要求。

### 系列

检查：是否像同一品牌、是否偏正方形、视觉重心是否处于相近水平、视觉重量与线面节奏是否平衡、是否存在明显过长/过重/过空/过细成员、是否存在语义碰撞、是否依赖同一外框模板制造统一、是否错误混入橙色或其他颜色。
